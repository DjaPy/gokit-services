...
// Server is an HTTP server implementing service.Service and service.Shutdown.
type Server struct {
	host              string
	port              int
...
// Start implements service.Service. It binds and serves until the server is
// shut down. When ctx is canceled it begins a graceful shutdown that drains
// in-flight requests without a deadline of its own — the shutdown deadline is
// supplied by Stop's context (see Stop). A standalone caller that needs a
// bounded shutdown should call Stop rather than rely on ctx cancellation alone.
...
// gracefulShutdown drains in-flight requests, closing listeners first. It is
// guarded by shutdownOnce so the ctx-cancel path in Start and Stop share a
// single, idempotent graceful shutdown. It carries no deadline of its own —
// Stop bounds it by forcing connections closed when its context expires.
...
// Stop implements service.Shutdown. It gracefully drains in-flight requests; if
// ctx expires (or is already done) it forces the remaining connections closed
// and returns ctx.Err(). Repeated calls are safe.
